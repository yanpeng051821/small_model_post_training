...
| Week | 主题 | 状态 | 核心产出 |
|---|---|---|---|
| 1 | 源码地图与复现合同 | 进行中 | pinned source、entrypoint、数据与指标合同 |
| 2 | 环境、推理与 B0 baseline | 未开始 | 可重复环境、训练前指标 |
| 3 | 数据审计与 SFT smoke test | 未开始 | 固定数据子集、一次最小训练闭环 |
| 4 | Reasoning SFT 缩放复现 | 未开始 | B1 checkpoint、完整训练日志 |
| 5 | SFT 评测与失败分析 | 未开始 | B0/B1 对照、失败样本分类 |
