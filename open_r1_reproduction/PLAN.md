...
最后更新：2026-07-23
状态：Week 1 路线合同已完成；进入 Week 2 setup
...
核心产出不是最高分，而是一条可重复、可解释、可比较的 baseline。源码和算法需要读懂；Trainer、GRPO loss 与 rollout engine 暂不从零重写。该路线不宣称完整复现 DeepSeek-R1 的大规模多阶段训练。
